...
//   • "3f.xyz"            → strip to clean domain → typeahead("3f.xyz")
//   • "https://3f.xyz/x"  → strip scheme/path → typeahead("3f.xyz")
// After enrichment, if user typed a domain we hoist any result whose website
// domain matches exactly to position 1 — protects against Harmonic re-ranking.
// (Note: Harmonic's REST /companies?website_domain= returns [] for major
// companies — verified via /api/harmonic/domain-lookup. Typeahead is the only
...
app.get('/api/harmonic/typeahead', async (req, res) => {
  ...
  // 3F, "https://3f.xyz/" reduces to "3f.xyz" so it matches too. Bare names
  // pass through unchanged.
  const nameQ = isUrlish ? domain : qRaw.trim();
  const headers = { apikey: harmonicKey };

  try {
    const tUrl = `${HARMONIC_BASE}/search/typeahead?query=${encodeURIComponent(nameQ)}&size=${size}`;
    const tRes = await fetch(tUrl, { headers });
    if (!tRes.ok) {
      console.error(`[Typeahead] HTTP ${tRes.status} for "${nameQ}"`);
      return res.json({ results: [] });
    }
    const tData = await tRes.json();
    const raw = Array.isArray(tData) ? tData : (tData.results || []);

    // Extract unique numeric IDs, preserving Harmonic's ranking.
    const ids = [];
    const seen = new Set();
    for (const r of raw) {
      const urn = r.entity_urn || r.entityUrn || r.urn || '';
      const id = parseInt(r.id || urn.split(':').pop());
      if (!id || seen.has(id)) continue;
      seen.add(id);
      ids.push(id);
      if (ids.length >= Math.max(size, 6)) break;
    }

    if (ids.length === 0) {
      console.log(`[Typeahead] "${qRaw}" (urlish=${isUrlish}, q=${nameQ}) → 0 results`);
      return res.json({ results: [] });
    }

    // Enrich via fast REST per-id fetch
    const fetched = await Promise.allSettled(
      ids.map(id => fetch(`${HARMONIC_BASE}/companies/${id}`, { headers })
    ...
    // If the user typed a domain, hoist any exact domain-match to position 1.
    // Cheap insurance against Harmonic ranking the wrong "X.com.vc" first.
    if (isUrlish && domain && enriched.length > 1) {
      const matchIdx = enriched.findIndex(c => String(c.domain || '').toLowerCase() === domain);
      if (matchIdx > 0) {
        const [hit] = enriched.splice(matchIdx, 1);
        enriched.unshift(hit);
        console.log(`[Typeahead] hoisted "${hit.name}" (domain=${hit.domain}) from #${matchIdx + 1} to #1`);
      }
    }
    ...
    console.log(`[Typeahead] "${qRaw}" (urlish=${isUrlish}, q=${nameQ}) → ${filtered.length}: ${filtered.map(e => e.name).join(', ')}`);
...
app.post('/api/recurring-scan', async (req, res) => {
  ...
  const anthropicKey = req.headers['x-anthropic-key'] || req.body?.anthropicKey || process.env.ANTHROPIC_API_KEY;
  const harmonicKey = process.env.HARMONIC_API_KEY;
...
${batchText}`;
    ...
    // Corpus-size signals (informational only; never aborts a scan):
    // <2500 = warning (corpus too narrow), >=10000 = ideal coverage.
    scan.stats.corpusSize = filtered.length;
    scan.stats.corpusWarning = filtered.length < 2500;
    scan.stats.corpusIdeal = filtered.length >= 10000;
    if (scan.stats.corpusWarning) safeWrite(`: ⚠️ Corpus warning: only ${filtered.length} candidates (<2500); results may be narrow\n\n`);
    else if (scan.stats.corpusIdeal) safeWrite(`: ✅ Corpus ideal: ${filtered.length} candidates (≥10000)\n\n`);
...
${batchText}`;
    ...
    const finalResults = {
      ...
      screenAnalysis,
      stats: scan.stats,
      // Corpus-size signals — informational; <2500 = narrow, >=10000 = ideal.
      corpusSize: scan.stats.corpusSize ?? null,
      corpusWarning: scan.stats.corpusWarning ?? false,
      corpusIdeal: scan.stats.corpusIdeal ?? false,
...
// ==========================================
// WEEKLY PER-PARTNER SEARCH
// 7 partners, each with stored prompt + filters. Cron runs Monday 09:00 UTC.
// Each enabled partner's search calls the recurring-scan pipeline (tier=weeklyPartner),
// pushing top 5 to the DD vetting pipeline.
// ==========================================
...
function defaultPartnerConfig(seed) {
  return {
    ...
    context: '',
    filters: {
      sectors: [], stages: [], geos: [], models: [],
      signals: [], maxRaised: '', maxValuation: '',
      foundedAfter: '', minTeam: '', maxTeam: '',
...
function loadPartnerSearches() {
  let state;
  try {
    state = JSON.parse(fs.readFileSync(PARTNER_SEARCHES_FILE, 'utf8'));
  } catch {
    state = { partners: {} };
  }
  ...
  for (const seed of PARTNER_SEED) {
    if (!state.partners[seed.id]) {
      ...
      const dflt = defaultPartnerConfig(seed);
      // Fill in any missing top-level field.
      ...
      if (!p.filters || typeof p.filters !== 'object') { p.filters = dflt.filters; mutated = true; }
        ...
        for (const k of Object.keys(dflt.filters)) {
...
function savePartnerSearches(state) {
  try { fs.writeFileSync(PARTNER_SEARCHES_FILE, JSON.stringify(state, null, 2)); }
  catch (e) { console.error('[PartnerSearches] save error:', e.message); }
}
...
app.put('/api/partner-searches/:id', (req, res) => {
  ...
  // Filters merge — only known keys.
  if (body.filters && typeof body.filters === 'object') {
    for (const k of Object.keys(current.filters)) {
...
// Cheaper than refactoring the 900-line handler — the existing route already does
// everything we need (Sonnet screen → Opus deep → vetting push, filterBackburn, etc).
// We only need the scanId, which the SSE stream emits in its first data: line.
async function triggerRecurringScanLoopback(body) {
  const port = process.env.PORT || 3001;
  const url = `http://127.0.0.1:${port}/api/recurring-scan`;
  // node-fetch v3 is ESM-only; use dynamic import.
  const { default: fetch } = await import('node-fetch');
  const resp = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-scan-tier': 'weeklyPartner' },
    body: JSON.stringify(body),
  });
  // The endpoint is SSE; the very first `data:` line is the scanId. Read incrementally.
  ...
  await new Promise((resolve) => {
    const onData = (chunk) => {
      ...
      if (m) {
        try { const parsed = JSON.parse(m[1]); if (parsed.scanId) { scanId = parsed.scanId; resolve(); } } catch {}
      }
      // Also resolve if we hit an error event.
      if (/data:\s*\{[^\n]*"error"/.test(buf)) resolve();
    };
    const onEnd = () => resolve();
...
app.post('/api/partner-searches/:id/run', async (req, res) => {
  ...
  const state = loadPartnerSearches();
  const partner = state.partners[id];
  if (!partner.searchPrompt?.trim()) return res.status(400).json({ error: 'partner has no searchPrompt configured' });
...
// Sweeper — periodically updates lastRunResultCount/lastCorpusSize for partners whose
// last scan has just finished. Cheap; runs every 60s.
setInterval(() => {
  try {
    const state = loadPartnerSearches();
    let mutated = false;
    for (const partner of Object.values(state.partners)) {
      if (!partner.lastRunScanId) continue;
      const scan = global._deepScans?.[partner.lastRunScanId];
      if (!scan) continue;
      if (scan.status !== 'done') continue;
      const stats = scan.stats || {};
      const newCount = stats.topResults ?? stats.ddPushed ?? 0;
      const newCorpus = stats.corpusSize ?? 0;
      if (partner.lastRunResultCount !== newCount || partner.lastCorpusSize !== newCorpus) {
        partner.lastRunResultCount = newCount;
        partner.lastCorpusSize = newCorpus;
        mutated = true;
      }
    }
    if (mutated) savePartnerSearches(state);
