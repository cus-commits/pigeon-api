  ...
  try {
    ...
    try {
      ...
      if (old && old.status && old.status !== 'idle') {
        ...
        if (old.status === 'scanning') { old.status = 'interrupted'; old.progress = `Interrupted — was at: ${old.progress || 'unknown'}`; }
...
function saveScansState() {
  try {
    ...
    for (const [id, s] of Object.entries(global._deepScans || {})) {
      toSave[id] = { id: s.id, status: s.status, progress: s.progress, stats: { ...(s.stats || {}) }, startedAt: s.startedAt, tier: s.tier, user: s.user || 'Mark', options: s.options };
    }
...
const SCAN_TIERS = {
  scout:    { name: 'Quick Scout',    cost: '$5',  budget: 5,   ddPush: 3,  preModel: 'claude-haiku-4-5-20251001', preMax: 1500, screenModel: 'claude-sonnet-4-20250514', screenMax: 200, deepModel: 'claude-sonnet-4-20250514', deepMax: 25,  preBatch: 200, screenBatch: 80, deepBatch: 10, desc: 'Haiku pre-screen → Sonnet scoring → Sonnet deep (budget-capped) → top to DD' },
  standard: { name: 'Standard',      cost: '$12', budget: 12,  ddPush: 8,  preModel: 'claude-sonnet-4-20250514',  preMax: 1500, screenModel: 'claude-sonnet-4-20250514', screenMax: 200, deepModel: 'claude-opus-4-6',          deepMax: 80,  preBatch: 120, screenBatch: 50, deepBatch: 15, desc: 'Sonnet pre-screen → Sonnet scoring → Opus deep (budget-capped) → top to DD' },
  deep:     { name: 'Deep Dive',     cost: '$25', budget: 25,  ddPush: 15, preModel: 'claude-sonnet-4-20250514',  preMax: 5000, screenModel: 'claude-sonnet-4-20250514', screenMax: 500, deepModel: 'claude-opus-4-6',          deepMax: 150, preBatch: 120, screenBatch: 50, deepBatch: 15, desc: 'Sonnet all → Sonnet scoring → Opus deep (budget-capped) → top to DD' },
  sweep:    { name: 'Full Sweep',    cost: '$35', budget: 35,  ddPush: 25, preModel: 'claude-sonnet-4-20250514',  preMax: 9999, screenModel: 'claude-sonnet-4-20250514', screenMax: 999, deepModel: 'claude-opus-4-6',          deepMax: 300, preBatch: 120, screenBatch: 50, deepBatch: 15, desc: 'Sonnet full → Sonnet scoring → Opus deep (budget-capped) → top to DD' },
  maximum:  { name: 'Maximum',       cost: '$50', budget: 50,  ddPush: 40, preModel: 'claude-sonnet-4-20250514',  preMax: 9999, screenModel: 'claude-sonnet-4-20250514', screenMax: 999, deepModel: 'claude-opus-4-6',          deepMax: 500, preBatch: 120, screenBatch: 50, deepBatch: 15, desc: 'Full pipeline → Opus deep (budget-capped) → top to DD' },
};
